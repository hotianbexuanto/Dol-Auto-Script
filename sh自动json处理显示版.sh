...
update_progress_bar() {
    ...
    printf "\r\033[K[%-${width}s] %d%%" "$bar" "$percent"
}
...
echo -e "扫描文件和目录：\n[                                                  ] 0%"
{
    for ((i=0; i<total_files; i++)); do
        echo "${files[i]}"
        update_progress_bar 1 "$i" "$total_files"
    done
    echo
} > "$TEMP_FILE"
...
echo -e "处理图像文件：\n[                                                  ] 0%"
{
    ...
    for ((i=0; i<total_files; i++)); do
        if [[ "${files[i]}" == *.png || "${files[i]}" == *.jpg || "${files[i]}" == *.gif ]]; then
            ...
            update_progress_bar 2 "$img_processed" "$img_files"
        fi
    done
    echo
} > /dev/null  # 防止输出影响进度条
...
echo -e "处理其他文件：\n[                                                  ] 0%"
{
    ...
    for ((i=0; i<total_files; i++)); do
        if [[ -f "${files[i]}" && "${files[i]}" != *.png && "${files[i]}" != *.jpg && "${files[i]}" != *.gif ]]; then
            ...
            update_progress_bar 3 "$other_processed" "$other_files"
        fi
    done
    echo
} > /dev/null
...
echo -e "处理目录：\n[                                                  ] 0%"
{
    ...
    for ((i=0; i<total_files; i++)); do
        if [[ -d "${files[i]}" ]]; then
            ...
            update_progress_bar 4 "$dir_processed" "$dir_files"
        fi
    done
    echo
} > /dev/null
...
echo -e "保存目录结构：\n[                                                  ] 0%"
{
    ...
    for ((i=0; i<total_files; i++)); do
        if [[ "${files[i]}" == *.png || "${files[i]}" == *.jpg || "${files[i]}" == *.gif ]]; then
            ...
            update_progress_bar 5 "$save_processed" "$save_total"
        fi
    done
