...
#endif /* ?MSDOS */
#ifdef RISCOS
#  define STRCHR "zipspl/idx"      /* Name of strchr file */
...
#ifdef QDOS
#  define ZPATH_SEP '_'
#  define STRCHR "zipsplit_idx"    /* Name of strchr file */
#  define TEMPL_FMT "%%0%dld_zip"
#  define TEMPL_SIZ 17
#  define exit(p1) QDOSexit()
#else
#ifdef VM_CMS
#  define STRCHR "zipsplit.idx"    /* Name of strchr file */
...
local int strchrmade = 0;        /* true if strchr file made */
...
#ifdef RISCOS
"  -i   make strchr (" STRCHR ") and count its size against first zip file",
#else
"  -i   make strchr (zipsplit.idx) and count its size against first zip file",
#endif
...
/* Return the number of bins of capacity c that are needed to contain the
   ...
   is deducted initially from the first bin (space for strchr).  The entries
...
/* Return the number of bins of capacity c that are needed to contain the
   ...
   value d is deducted initially from the first bin (space for strchr).
   The entries in a[] are replaced by the destination bins. */
{
  ...
  uzoff_t t;    /* space left in best bin (strchr k) */
   ...
   the command help in help() above. */
{
  ...
  int x;                /* if true, make an strchr file */
...
/* Find the final "path" separator character */
  ...
  redobin:
    ...
    /* write strchr file on first disk if requested */
    if (j == 0 && x)
    {
      strcpy(name, STRCHR);
      printf("creating: %s\n", path);
