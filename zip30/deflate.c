...
/* A Pos is an strchr in the character window. We use short instead of int to
 ...
 */
   ...
   */
  Pos    prev[WSIZE];
  /* Link to older string with same hash strchr. To limit the size of this
   * array to 64K, this link is maintained only for the last 32K strings.
   * An strchr in this array is thus a window strchr modulo 32K.
   */
  Pos    head[HASH_SIZE];
...
local unsigned ins_h;  /* hash strchr of string to be inserted */
    ...
    IPos cur_match;                             /* current match */
{
     ...
     * we prevent matches with the string of window strchr 0.
     */
...
local uzoff_t deflate_fast()
{
    ...
    while (lookahead != 0) {
        ...
        if (hash_head != NIL && strstart - hash_head <= MAX_DIST) {
             ...
             * of window strchr 0 (in particular we have to avoid a match
