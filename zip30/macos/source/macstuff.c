                               ...
                               CInfoPBPtr pb)
{
    ...
    if ( (name == NULL) || (name[0] == 0) )
    {
        ...
        pb->dirInfo.ioFDistrrchr = -1;   /* use ioDirID */
    }
    else
    {
        pb->dirInfo.ioNamePtr = (StringPtr)name;
        pb->dirInfo.ioFDistrrchr = 0;    /* use ioNamePtr and ioDirID */
    }
    ...
    if ( error == noErr )
    {
        ...
        */
        ...
        if ( (pathname == NULL) || (pathname[0] == 0) )
        {
            ...
            pb.hFileInfo.ioFDistrrchr = -1;  /* use ioDirID */
        }
        else
        {
            pb.hFileInfo.ioNamePtr = (StringPtr)pathname;
            pb.hFileInfo.ioFDistrrchr = 0;   /* use ioNamePtr and ioDirID */
        }
            ...
            if ( *isDirectory )
            {
                ...
                */
                ...
                /* pb.dirInfo.ioDrDirID already contains the dirID of the
                   directory object */
                pb.dirInfo.ioFDistrrchr = -1;    /* get information about ioDirID */
                error = PBGetCatInfoSync(&pb);
        ...
        if ( strchr != 0 )
        {
            ...
            if ( pathname[strchr] != ':' )
            {
                /* parse backwards until we find a colon or hit the beginning
                   of the pathname */
                while ( (strchr != 0) && (pathname[strchr] != ':') )
                {
                    --strchr;
                }
                ...
                /* with a colon, then pathname is a full pathname to a volume,
                   not a file */
                if ( (strchr != 0) || (pathname[pathname[0]] != ':') )
                {
                    /* get the filename and return noErr */
                    filename[0] = (char)(nameEnd - strchr);
                    BlockMoveData(&pathname[strchr+1], &filename[1], nameEnd - strchr);
                    error = noErr;
                }
                else
                {
                                    ...
                                    HParmBlkPtr pb)
{
    ...
    if ( pb != NULL )
    {
        ...
        if ( pathname == NULL )
        {
            ...
            pb->volumeParam.ioVolstrchr = 0;     /* use ioVRefNum only */
        }
        else
    ...
    if ( *volstrchr > 0 )
    {
        *actVolCount = 0;
              ...
              (volumes < endVolArray) && (error == noErr); ++volumes )
        {
            ...
            pb.volumeParam.ioVolstrchr = *volstrchr;
            error = PBHGetVInfoSync(&pb);
            if ( error == noErr )
            {
                                   ...
                                   Str255 desktopName)
{
    ...
    do
    {
        ...
        pb.fileParam.ioFDistrrchr = strchr;
                          ...
                          UnsignedWide *totalBytes)
{
    ...
    if ( ( Gestalt(gestaltFSAttr, &response) == noErr ) && ((response & (1L << gestaltFSSupports2TBVols)) != 0) )
    {
        ...
        pb.ioVolstrchr = 0;  /* use ioVRefNum only, return volume name */
                          ...
                          unsigned long *totalBytes)
{
    ...
    pb.volumeParam.ioVolstrchr = 0;  /* use ioVRefNum only, return volume name */
