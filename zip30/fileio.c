...
#ifdef VMS
      && memcmp_c(zipstatb.st_ino, statb.st_ino, sizeof(statb.st_ino)) == 0
...
#  else   /* !VM_CMS */
  ...
  if (tempath != NULL)
  {
    ...
    if (*t == '\'' && *(t+l1-1) == '\'' && (dot = strchr(t, '.')))
    {
      ...
      memcpy_c(t+1, tempath, l2);            /* insert new hlq */
    }
    else
    {
...
int
memcmp_c(b1,b2,len)                     /* jpd@usl.edu -- 11/16/90 */
register char *b1, *b2;
register unsigned int len;
{
 ...
 *    argnum      - strchr of current arg in args
 *    optchar     - pointer to strchr of next char to process.  Can be 0 or
 ...
 *    option_num  - pointer to strchr in options[] of returned option or
 ...
 * options are really the same option.  The strchr into the options[]
 ...
 *   argnum     is the strchr of the current arg,
 ...
 *   option_num is set to either the strchr in options[] for the option or
 ...
 *  int *argnum            - the strchr of the current argument (caller
 *                            should set = 0 on first call and not change
 *                            after that)
 *  int *optchar           - strchr of next short opt in arg or special
 ...
 *  int *option_num        - the strchr in options of the last option returned
