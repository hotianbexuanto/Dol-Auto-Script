...
def list_files_and_subdirectories(directory, output_dict, mode='all'):
...
# 选择模式
scan_mode = input("请选择扫描模式（输入1仅扫描png和gif文件，输入2扫描所有文件，默认2）:")
if scan_mode == '1':
    scan_mode = 'img_only'
else:
    scan_mode = 'all'
...
# 根据选择的模式列出 img 文件夹下的文件并分类
list_files_and_subdirectories('img', output_dict, mode=scan_mode)
